...
<body>
	<form action="user.html" method="post" name="userForm" id="userForm">
		<table width="100%" border="0" cellpadding="0" cellspacing="0"
			...
			<c:choose>
				<c:when test="${not empty userList}">
					<c:forEach items="${students}" var="user" varStatus="vs">
						<tr class="main_info">
							<td><input type="checkbox" name="userIds"
								id="userIds${user.userId }" value="${user.userId }" /></td>
							<td>${vs.index+1}</td>
							...
							<td><a href="javascript:editUser(${user.userId });">修改</a> |
								<a href="javascript:delUser(${user.userId });">删除</a> | <a
								href="javascript:editRights(${user.userId });">权限</a></td>
						</tr>
					</c:forEach>
				</c:when>
				<c:otherwise>
		...
		<div class="page_and_btn">
			<div>
				<a href="javascript:addUser();" class="myBtn"><em>新增</em></a> <a
					href="javascript:exportUser();" class="myBtn"><em>导出</em></a>
			</div>
			${user.page.pageStr }
		</div>
	</form>
